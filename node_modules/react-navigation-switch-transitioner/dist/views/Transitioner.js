...
const getStateForNavChange = (props, state) => {
  ...
  const descriptor = props.descriptors[toKey] || state.descriptors[toKey] || state.transitioningFromDescriptors[toKey];
  ...
  const fromDescriptor = props.descriptors[fromKey] || state.descriptors[fromKey] || state.transitioningFromDescriptors[fromKey];
  const { options: fromOptions } = fromDescriptor;
  const createFromTransition = fromOptions.createTransition || defaultCreateTransition;
...
export default class Transitioner extends React.Component {
  ...
  async _startTransition() {
    ...
    const fromDescriptor = descriptor[fromKey] || transitioningFromDescriptors[fromKey];
    ...
    await Promise.all([runFrom(fromTransition, transitioningFromState, navState), run(transition, transitioningFromState, navState)]);
  ...
  render() {
    ...
    const incomingDescriptor = descriptors[activeKey] || transitioningFromDescriptors[activeKey];
    const incomingOptions = incomingDescriptor && incomingDescriptor.options;
    const incomingHasManagedTransition = incomingOptions && incomingOptions.runTransition && incomingOptions.createTransition;
    ...
    const outgoingHasManagedTransition = outgoingOptions && outgoingOptions.runTransition && outgoingOptions.createTransition;
    const outgoingTransition = oldKey && transitions[oldKey];
    const OutgoingScreen = outgoingDescriptor && outgoingDescriptor.getComponent();
    ...
    const screens = [<IncomingScreen {...incomingProps} navigation={incomingDescriptor.navigation} key={activeKey} />];

    if (OutgoingScreen) {
      const outgoingWithProps = <OutgoingScreen {...outgoingProps} navigation={outgoingDescriptor.navigation} key={oldKey} />;
    ...
    return <Animated.View style={[...style, { ...backScreenStyles }]} pointerEvents="auto">
